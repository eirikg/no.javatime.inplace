...
//				}
	...
	private Collection<Bundle> removeTransitionErrorClosures(Collection<Bundle> initialBundleSet) {
		...
		for (Bundle errorBundle : bErrorDepClosures) {
			...
			TransitionError transitionError = bundleTransition.getBuildTransitionError(errorBundle);
			if (null != errorProject
					&& (transitionError == TransitionError.BUILD_MODULAR_WORKSPACE_DUPLICATE || transitionError == TransitionError.BUILD_CYCLE)) {
				if (null == bundles) {
