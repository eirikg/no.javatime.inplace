...
/**
 * Bundle listener acting on events received from the OSGI framework. Bundle events are initiated by
 * bundle operations in {@link BundleCommandImpl}, from an external source(e.g. the OSGi Bundle
 * Console) or from the framework itself.
 * <p>
 ...
 * itself and correct the internal state of the machine based commands initiated from external
 * sources
 * <p>
 ...
 * <ol>
 * <li>When a bundle is started due to "on demand class loading" for lazy activated bundles
 * <li>A stop transition is generated when an exception occurs during the start operation
 * </ol>
 * <p>
 ...
 * <li>Uninstalling a bundle in state resolved. Both transitions are uninstall, where the first moves
 * the bundle to state installed and the second to state uninstalled
...
public class BundleStateEvents implements SynchronousBundleListener {
	 ...
	 * transition.
	 * <p>
	 * An exception is when a bundle with lazy activation policy is started due to "on demand" class
	 * loading. This is a transition generated by the framework, and the maintained state of the
	 * bundle is adjusted after the fact from a lazy activation state to an active state.
	 * <p>
	...
	public void bundleChanged(BundleEvent event) {
		...
		// @formatter:off
		/*
		 * Install a bundle with initial state uninstalled and terminal state installed.
		 * For external commands force the bundle to state installed.
		 */
		 ...
		 * installed and moves the bundle to the terminal state uninstalled.
		 */
		 ...
		 * start routine which invoked the start command. For lazy loaded bundles force a roll back
		 * (by using an explicit commit).
		 */
		...
		default: {
			node.getState().external(node, event, StateFactory.INSTANCE.stateLess,
					Transition.EXTERNAL);
		}
		} // switch
		// @formatter:on
