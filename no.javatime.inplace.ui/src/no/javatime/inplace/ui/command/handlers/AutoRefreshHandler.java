...
public class AutoRefreshHandler extends AbstractHandler implements IElementUpdater {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		Category.setState(Category.autoRefresh, !state);
		return null;
	}

	@Override
	public void updateElement(UIElement element, @SuppressWarnings("rawtypes") Map parameters) {
		Command command = BundleMenuActivationHandler.setCheckedMenuEntry(Category.autoRefresh, commandId);
		if (null != command) {
			element.setChecked(Category.getState(Category.autoRefresh));
		}
	}
