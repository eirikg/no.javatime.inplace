...
public class UpdateClassPathOnActivateHandler extends AbstractHandler implements IElementUpdater {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		Category.setState(Category.updateClassPathOnActivate, !state);
		return null;
	}

	@Override
	public void updateElement(UIElement element, @SuppressWarnings("rawtypes") Map parameters) {
		Command command = BundleMenuActivationHandler.setCheckedMenuEntry(Category.updateClassPathOnActivate,
				commandId);
		if (null != command) {
			element.setChecked(Category.getState(Category.updateClassPathOnActivate));
		}
	}
