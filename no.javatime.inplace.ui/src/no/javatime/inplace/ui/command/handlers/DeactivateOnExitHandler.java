...
public class DeactivateOnExitHandler extends AbstractHandler implements IElementUpdater {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		Category.setState(Category.deactivateOnExit, !prevState);
		return null;
	}

	@Override
	public void updateElement(UIElement element, @SuppressWarnings("rawtypes") Map parameters) {
		Command command = BundleMenuActivationHandler.setCheckedMenuEntry(Category.deactivateOnExit, commandId);
		if (null != command) {
			element.setChecked(Category.getState(Category.deactivateOnExit));
		}
	}
