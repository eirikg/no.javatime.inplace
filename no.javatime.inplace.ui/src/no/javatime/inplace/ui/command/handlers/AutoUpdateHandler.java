...
public class AutoUpdateHandler extends AbstractHandler implements IElementUpdater {
	...
	public static String stateId = "no.javatime.inplace.command.autoupdate.toggleState";
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		cmdStore.setIsUpdateOnBuild(stateVal);
		try {
			cmdStore.flush();
	...
	public void updateElement(UIElement element, @SuppressWarnings("rawtypes") Map parameters) {
		CommandOptions cmdStore = Activator.getDefault().getPrefService();
		element.setChecked(cmdStore.isUpdateOnBuild());
	}
	...
	public boolean isEnabled() {
		...
		if (null != service) {
			...
			Boolean stateVal = (Boolean) state.getValue();
			Boolean storeVal = cmdStore.isUpdateOnBuild();
