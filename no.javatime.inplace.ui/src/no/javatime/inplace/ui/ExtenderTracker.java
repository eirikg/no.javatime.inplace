	...
	public Collection<Extender<?>> addingBundle(Bundle bundle, BundleEvent event) {
		...
		try {
			...
			if (null != serviceName) {
				...
				trackExtender(bundle, TogglePolicy.class.getName(), new BundleExecutorServiceFactory(headers.get(TogglePolicy.TOGGLE_POLICY_SERVICE)), null);
				trackExtender(bundle, UpdateBundleClassPath.class.getName(), new BundleExecutorServiceFactory(headers.get(UpdateBundleClassPath.UPDATE_BUNDLE_CLASS_PATH_SERVICE)), null);
				resourceStateExtender = trackExtender(bundle, ResourceState.class.getName(), headers.get(ResourceState.RESOURCE_STATE_SERVICE), null);
				serviceName = headers.get(BundleExecutorEventManager.BUNDLE_EXECUTOR_EVENT_MANAGER_SERVICE);
				if (null != serviceName) {
					bundleExecManagerExtender = trackExtender(bundle, BundleExecutorEventManager.class.getName(), serviceName, null);
				}
			}
			...
			if (null != serviceName) {
				trackExtender(bundle, DependencyDialog.class.getName(), serviceName, null);
			}
			serviceName = headers.get(BundleLogView.BUNDLE_LOG_VIEW_SERVICE);
			if (null != serviceName) {
				trackExtender(bundle, BundleLogView.class.getName(), serviceName, null);
			}
			serviceName = headers.get(BundleConsoleFactory.BUNDLE_CONSOLE_SERVICE);
			if (null != serviceName) {
				trackExtender(bundle, BundleConsoleFactory.class.getName(), serviceName, null);
			}
