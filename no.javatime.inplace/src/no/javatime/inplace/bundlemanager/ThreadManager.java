...
/** 
 * See: http://www.drdobbs.com/jvm/multithreading-java-osgi/191601642?pgno=1
 * 
 ...
 * Interruptable interface, which is an extension of Runnable.
 * Typical use is:
 ...
 *   tm.stopAll();
 * No guarantees are made that Interruptable.run() is invoked before
 * Interruptable.interrupt(). They may be called in the other order when, 
 ...
 * that if a call to tm.run() for some Interruptable returns normally then
 * a call to tm.stopAll() will result in a call to Interruptable.interrupt() 
 * for that Interruptable. The implementation of Interruptable must handle 
 * run() and interrupt() being called in either order.
 * @see ThreadManager.Interruptable
 */
...
class ThreadManager {
    ...
    abstract static class Interruptable implements Runnable {
   ...
   /* ThreadManager schedules runnables of this type, not of Interruptable. */
    private class ManagedRunnable implements Runnable {
        private Interruptable target;
        ManagedRunnable( Interruptable target ) {
            this.target = target;
        }
        public void run() {
            try {
    ...
    /** Creates and starts a new thread for the given Interruptable. */
    public void run( final Interruptable target, String name ) {
        run( target, name, 0 );
    }
    /** Creates and starts a new thread for the given Interruptable.
    ...
    public void run( final Interruptable target, String name, 
                                                    int priorityBoost ) {
        // We wrap the given Interruptable with a new one that sets the
