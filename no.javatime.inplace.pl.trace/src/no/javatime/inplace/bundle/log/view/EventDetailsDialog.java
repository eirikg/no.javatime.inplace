...
public class EventDetailsDialog extends TrayDialog {
	...
	private void resetChildIndex() {
		...
		BundleLogEntryImpl thisEntry = (BundleLogEntryImpl) entry;

		for (int i = 0; i < entryChildren.length; i++) {
			if (entryChildren[i] instanceof BundleLogEntryImpl) {

				BundleLogEntryImpl traceLogEntry = (BundleLogEntryImpl) entryChildren[i];

				if (traceLogEntry == thisEntry) {
					childIndex = i;
					return;
				}
			}
	...
	protected void backPressed() {
		if (childIndex > 0) {
			...
			if (parentEntry instanceof BundleLogEntryImpl) {
				entry = parentEntry;
				if (isChild(entry)) {
		...
		if (sortType == LogView.DATE) {
			comparator = new Comparator() {
				public int compare(Object e1, Object e2) {
					Date date1 = ((BundleLogEntryImpl) e1).getDate();
					Date date2 = ((BundleLogEntryImpl) e2).getDate();
	...
	public void updateProperties() {
		...
		if (entry instanceof BundleLogEntryImpl) {
			BundleLogEntryImpl traceLogEntry = (BundleLogEntryImpl) entry;

			String strDate = dateFormat.format(traceLogEntry.getDate());
			dateLabel.setText(strDate);
			plugInIdLabel.setText(traceLogEntry.getPluginId());
			severityImageLabel.setImage(labelProvider.getColumnImage(entry, 0));
			severityLabel.setText(traceLogEntry.getSeverityText());
			msgText.setText(traceLogEntry.getMessage() != null ? traceLogEntry.getMessage() : ""); //$NON-NLS-1$
			String stack = traceLogEntry.getStack();
			...
			if (traceLogEntry.getSession() != null) {
				String session = traceLogEntry.getSession().getSessionData();
				if (session != null) {
	...
	private void updateButtons() {
		...
		if (isChild(entry)) {
			boolean canGoToParent = (entry.getParent(entry) instanceof BundleLogEntryImpl);
	...
	private void findNextSelectedChild(AbstractEntry originalEntry) {
		...
		} else if (parentEntry instanceof BundleLogEntryImpl) {
			entry = parentEntry;
			setEntryChildren();
			resetChildIndex();
			isLastChild = childIndex == entryChildren.length - 1;
