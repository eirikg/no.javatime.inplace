...
package no.javatime.inplace.bundle.log.view;
...
import no.javatime.inplace.bundle.log.dl.AbstractEntry;
import no.javatime.inplace.bundle.log.dl.LogFilesManager;
import no.javatime.inplace.bundle.log.dl.LogReader;
import no.javatime.inplace.bundle.log.dl.LogSession;
import no.javatime.inplace.bundle.log.impl.BundleLogEntryImpl;
...
public class LogView extends ViewPart implements SynchronousLogListener, LogFilter {
	...
	protected static final String P_COLUMN_4 = "column5"; //$NON-NLS-1$
	...
	public void createPartControl(Composite parent) {
		...
		Activator.getDefault().getLogReaderService().addLogListener(this, this);
	...
	private void createActions() {
		...
		IMenuListener listener = new IMenuListener() {
			public void menuAboutToShow(IMenuManager manager) {
				...
				if ((selection.length > 0) && (selection[0].getData() instanceof BundleLogEntryImpl)) {
	...
	private Action createDeleteLogAction() {
		...
		action.setEnabled(fInputFile.exists() && fInputFile.equals(Activator.getDefault().getLogFile()));
		return action;
	}
		...
		PatternFilter filter = new PatternFilter() {
			protected boolean isLeafMatch(Viewer viewer, Object element) {
				if (element instanceof BundleLogEntryImpl) {
					BundleLogEntryImpl traceLogEntry = (BundleLogEntryImpl) element;
					String message = traceLogEntry.getMessage();
					String plugin = traceLogEntry.getPluginId();
					DateFormat dateFormat = DateFormat.getDateTimeInstance(DateFormat.SHORT, DateFormat.SHORT);
					String date = dateFormat.format(traceLogEntry.getDate());
	...
	private void createColumns(Tree tree) {
		...
		fColumn3.setText(Messages.LogView_column_state);
		fColumn3.setWidth(fMemento.getInteger(P_COLUMN_3).intValue());

		fColumn4 = new TreeColumn(tree, SWT.LEFT);
		fColumn4.setText(Messages.LogView_column_date);
		fColumn4.setWidth(fMemento.getInteger(P_COLUMN_4).intValue());
		fColumn4.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				...
				setColumnSorting(fColumn4, DATE_ORDER);
			}
		});
	...
	public void dispose() {
		writeSettings();
		Activator.getDefault().getLogReaderService().removeLogListener(this);
		fClipboard.dispose();
		if (fTextShell != null)
			fTextShell.dispose();
	...
	private void group(List entries) {
		if (fMemento.getInteger(P_GROUP_BY).intValue() == GROUP_BY_NONE) {
			...
			for (Iterator i = entries.iterator(); i.hasNext();) {
				BundleLogEntryImpl entry = (BundleLogEntryImpl) i.next();
	...
	private void limitEntriesCount() {
		...
		Comparator dateComparator = new Comparator() {
			public int compare(Object o1, Object o2) {
				Date l1 = ((BundleLogEntryImpl) o1).getDate();
				Date l2 = ((BundleLogEntryImpl) o2).getDate();
				if ((l1 != null) && (l2 != null)) {
	 ...
	 */
	protected Group getGroup(BundleLogEntryImpl entry) {
	...
	public void logged(LogEntry entry) {
		...
		if (context instanceof IBundleStatus) {
			logging((IBundleStatus) context, ((IBundleStatus) context).getPlugin());
		}
	}

	public void logging(IBundleStatus status, String plugin) {
		if (!isLogOpen())
			return;

		if (batchEntries) {
			// create TraceLogEntry immediately to don't loose IStatus creation date.
			BundleLogEntryImpl entry = createLogEntry(status);
			...
			protected IStatus run(IProgressMonitor monitor) {
				for (int i = 0; i < batchedEntries.size(); i++) {
					if (!monitor.isCanceled()) {
						BundleLogEntryImpl entry = (BundleLogEntryImpl) batchedEntries.get(i);
						pushEntry(entry);
	...
	private BundleLogEntryImpl createLogEntry(IBundleStatus status) {
		BundleLogEntryImpl entry = new BundleLogEntryImpl(status, currentSession);
		if (status.getException() instanceof CoreException) {
			IStatus coreStatus = ((CoreException) status.getException()).getStatus();
			if (coreStatus != null) {
				BundleLogEntryImpl childEntry = createLogEntry(new BundleStatus(coreStatus));
	...
	private synchronized void pushEntry(BundleLogEntryImpl entry) {
			...
			display.asyncExec(new Runnable() {
				public void run() {
					if (!fTree.isDisposed()) {
						...
						fDeleteLogAction.setEnabled(fInputFile.exists() && fInputFile.equals(Activator.getDefault().getLogFile()));
	...
	private void addDragSource() {
		...
		source.addDragListener(new DragSourceAdapter() {
			...
			public void dragStart(DragSourceEvent event) {
				...
				if (!(entry instanceof BundleLogEntryImpl)) {
					event.doit = false;
					return;
				}
			}
	...
	void onMouseHover(Event e) {
		...
		String message = null;
		if (item.getData() instanceof BundleLogEntryImpl) {
			message = ((BundleLogEntryImpl) item.getData()).getStack();
			...
			if (session.getDate() != null) {
				DateFormat formatter = new SimpleDateFormat(BundleLogEntryImpl.F_DATE_FORMAT);
	...
	void onMouseMove(Event e) {
		...
		if (data instanceof BundleLogEntryImpl) {
			BundleLogEntryImpl entry = (BundleLogEntryImpl) data;
		...
		if (sortType == DATE) {
			fComparator = new Comparator() {
				public int compare(Object e1, Object e2) {
					long date1 = 0;
					long date2 = 0;
					if ((e1 instanceof BundleLogEntryImpl) && (e2 instanceof BundleLogEntryImpl)) {
						date1 = ((BundleLogEntryImpl) e1).getDate().getTime();
						date2 = ((BundleLogEntryImpl) e2).getDate().getTime();
			...
			fComparator = new Comparator() {
				public int compare(Object e1, Object e2) {
					if ((e1 instanceof BundleLogEntryImpl) && (e2 instanceof BundleLogEntryImpl)) {
						BundleLogEntryImpl entry1 = (BundleLogEntryImpl) e1;
						BundleLogEntryImpl entry2 = (BundleLogEntryImpl) e2;
	...
	private void writeViewSettings() {
		...
		instancePrefs.putInt(P_COLUMN_4, fMemento.getInteger(P_COLUMN_4).intValue());
	...
	public void sortByDateDescending() {
		setColumnSorting(fColumn4, DESCENDING);
	}
	...
	public boolean isCurrentLogSession(LogSession session) {
		return isLogOpen() && (currentSession != null) && (currentSession.equals(session));
	}
	...
	public boolean isLogOpen() {
		return (fInputFile.equals(Activator.getDefault().getLogFile()));
	}
	 ...
	 */
	public void setLog() {
		setLogFile(Activator.getDefault().getLogFile());
	}
