...
// --- Begin writing TraceLogEntry --
	...
	private synchronized void log(TraceLogEntry traceLogEntry) {
 ...
 */
	private void writeLog(int depth, TraceLogEntry entry) throws IOException {
		...
		if (children != null) {
			for (int i = 0; i < children.length; i++) {
				...
				if (logEntry instanceof TraceLogEntry) {
					writeLog(depth + 1, (TraceLogEntry) logEntry);
				}
			}
		}
	}
	 ...
	 * @throws IOException if any error occurs writing to the log
	 */
	private void writeEntry(int depth, TraceLogEntry entry) throws IOException {
		if (depth == 0) {
	 ...
	 * @throws IOException if any error occurs writing to the log
	 */
	private void writeMessage(TraceLogEntry entry) throws IOException {
		write(MESSAGE);
		writeSpace();
	 ...
	 * @throws IOException if any error occurs writing to the log
	 */
	private void writeState(TraceLogEntry entry) throws IOException {
	 ...
	 * @throws IOException if any error occurs writing to the log
	 */
	private void writeStack(TraceLogEntry entry) throws IOException {
	...
	public void logged(LogEntry entry) {
		...
		if (context instanceof IBundleStatus) {
			log(new TraceLogEntry((IBundleStatus) context));
			return;
		}
		if (context instanceof TraceLogEntry) {
			log((TraceLogEntry) context);
			return;
		}
	}
