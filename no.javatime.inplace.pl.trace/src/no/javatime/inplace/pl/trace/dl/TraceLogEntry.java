 ...
 */
public class TraceLogEntry extends AbstractEntry {
	 ...
	 */
	public TraceLogEntry(IBundleStatus status) {
		this(status, null);
	}
	 ...
	 * @param status an existing status to create a new entry from
	 */
	 public TraceLogEntry(IBundleStatus status, LogSession session) {
	 ...
	 * <li>The parent of this entry is not <code>null</code> and is a {@link TraceLogEntry}</li>
	 * </ul>
	 * @return the {@link LogSession} for this entry
	 */
	public LogSession getSession() {
		if ((session == null) && (parent != null) && (parent instanceof TraceLogEntry)) {
			return ((TraceLogEntry) parent).getSession();
		}
		return session;
	}
		...
		if (schildren.length > 0) {
			for (int i = 0; i < schildren.length; i++) {
				if (schildren[i] instanceof IBundleStatus) {
					addChild(new TraceLogEntry((IBundleStatus) schildren[i], session));
				}
			}
		}
	}
