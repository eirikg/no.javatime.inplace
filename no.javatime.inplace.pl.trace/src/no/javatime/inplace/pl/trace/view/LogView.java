...
package no.javatime.inplace.pl.trace.view;
...
import no.javatime.inplace.pl.trace.dl.AbstractEntry;
import no.javatime.inplace.pl.trace.dl.LogFilesManager;
import no.javatime.inplace.pl.trace.dl.LogReader;
import no.javatime.inplace.pl.trace.dl.LogSession;
import no.javatime.inplace.pl.trace.dl.TraceLogEntry;
import no.javatime.inplace.pl.trace.msg.Messages;
...
public class LogView extends ViewPart implements SynchronousLogListener, LogFilter {
	...
	private void createActions() {
		...
		IMenuListener listener = new IMenuListener() {
			public void menuAboutToShow(IMenuManager manager) {
				...
				if ((selection.length > 0) && (selection[0].getData() instanceof TraceLogEntry)) {
	...
	private void createViewer(Composite parent) {
		PatternFilter filter = new PatternFilter() {
			protected boolean isLeafMatch(Viewer viewer, Object element) {
				if (element instanceof TraceLogEntry) {
					TraceLogEntry traceLogEntry = (TraceLogEntry) element;
	...
	private void group(List entries) {
		if (fMemento.getInteger(P_GROUP_BY).intValue() == GROUP_BY_NONE) {
			...
			for (Iterator i = entries.iterator(); i.hasNext();) {
				TraceLogEntry entry = (TraceLogEntry) i.next();
	...
	private void limitEntriesCount() {
		...
		Comparator dateComparator = new Comparator() {
			public int compare(Object o1, Object o2) {
				Date l1 = ((TraceLogEntry) o1).getDate();
				Date l2 = ((TraceLogEntry) o2).getDate();
				if ((l1 != null) && (l2 != null)) {
	 ...
	 */
	protected Group getGroup(TraceLogEntry entry) {
	...
	public void logging(IBundleStatus status, String plugin) {
		...
		if (batchEntries) {
			...
			TraceLogEntry entry = createLogEntry(status);
			...
			protected IStatus run(IProgressMonitor monitor) {
				for (int i = 0; i < batchedEntries.size(); i++) {
					if (!monitor.isCanceled()) {
						TraceLogEntry entry = (TraceLogEntry) batchedEntries.get(i);
						pushEntry(entry);
	...
	private TraceLogEntry createLogEntry(IBundleStatus status) {
		TraceLogEntry entry = new TraceLogEntry(status, currentSession);
		if (status.getException() instanceof CoreException) {
			IStatus coreStatus = ((CoreException) status.getException()).getStatus();
			if (coreStatus != null) {
				TraceLogEntry childEntry = createLogEntry(new BundleStatus(coreStatus));
	...
	private synchronized void pushEntry(TraceLogEntry entry) {
	...
	private void addDragSource() {
		...
		source.addDragListener(new DragSourceAdapter() {
			...
			public void dragStart(DragSourceEvent event) {
				...
				if (!(entry instanceof TraceLogEntry)) {
					event.doit = false;
					return;
				}
			}
	...
	void onMouseHover(Event e) {
		...
		String message = null;
		if (item.getData() instanceof TraceLogEntry) {
			message = ((TraceLogEntry) item.getData()).getStack();
			...
			if (session.getDate() != null) {
				DateFormat formatter = new SimpleDateFormat(TraceLogEntry.F_DATE_FORMAT);
	...
	void onMouseMove(Event e) {
		...
		if (data instanceof TraceLogEntry) {
			TraceLogEntry entry = (TraceLogEntry) data;
		...
		if (sortType == DATE) {
			fComparator = new Comparator() {
				public int compare(Object e1, Object e2) {
					long date1 = 0;
					long date2 = 0;
					if ((e1 instanceof TraceLogEntry) && (e2 instanceof TraceLogEntry)) {
						date1 = ((TraceLogEntry) e1).getDate().getTime();
						date2 = ((TraceLogEntry) e2).getDate().getTime();
			...
			fComparator = new Comparator() {
				public int compare(Object e1, Object e2) {
					if ((e1 instanceof TraceLogEntry) && (e2 instanceof TraceLogEntry)) {
						TraceLogEntry entry1 = (TraceLogEntry) e1;
						TraceLogEntry entry2 = (TraceLogEntry) e2;
