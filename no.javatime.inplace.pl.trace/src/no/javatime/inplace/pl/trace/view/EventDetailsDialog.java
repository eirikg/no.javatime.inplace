...
public class EventDetailsDialog extends TrayDialog {
	...
	private void resetChildIndex() {
		...
		TraceLogEntry thisEntry = (TraceLogEntry) entry;

		for (int i = 0; i < entryChildren.length; i++) {
			if (entryChildren[i] instanceof TraceLogEntry) {

				TraceLogEntry traceLogEntry = (TraceLogEntry) entryChildren[i];
	...
	protected void backPressed() {
		if (childIndex > 0) {
			...
			if (parentEntry instanceof TraceLogEntry) {
				entry = parentEntry;
				if (isChild(entry)) {
		...
		if (sortType == LogView.DATE) {
			comparator = new Comparator() {
				public int compare(Object e1, Object e2) {
					Date date1 = ((TraceLogEntry) e1).getDate();
					Date date2 = ((TraceLogEntry) e2).getDate();
	...
	public void updateProperties() {
		...
		if (entry instanceof TraceLogEntry) {
			TraceLogEntry traceLogEntry = (TraceLogEntry) entry;
	...
	private void updateButtons() {
		...
		if (isChild(entry)) {
			boolean canGoToParent = (entry.getParent(entry) instanceof TraceLogEntry);
	...
	private void findNextSelectedChild(AbstractEntry originalEntry) {
		...
		} else if (parentEntry instanceof TraceLogEntry) {
			entry = parentEntry;
			setEntryChildren();
			resetChildIndex();
			isLastChild = childIndex == entryChildren.length - 1;
